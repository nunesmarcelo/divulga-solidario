...
function mostra_doacoes(){
   ...
   $('#conteudo_andamento')[0].innerHTML=loading;
...
function recebe_todas_doacoes(result){
    ...
    for (var i=0; i<result.length; i++){
        ...
        if(result[i].entregue == '1' || result[i].entregue == '1'){
              $("#conteudo_andamento")[0].innerHTML += htmlString; 
        }
    }
    if($('#conteudo_andamento').html()==""){
        $('#conteudo_andamento').html('<tr><td colspan="5" style="text-align:center;font-size:1.5em" class="load_tr">Nenhuma doação encontrada.</td></tr>');
    }    
}
